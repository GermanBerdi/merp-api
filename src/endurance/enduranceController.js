...
const enduranceController =
{
  ...
  create: async function(request, reply)
  {
    try
    {
      ...
      {
        reply.code(400).send("Ya existe un endurance con name " + endurance.name);
        return;
      }
  ...
  delete: async function (request, reply)
  {
    try 
    {
      ...
      {
        reply.code(400).send("El Id " + enduranceId + " no tiene un longitud válida");
        return;
      }
      ...
      {
        reply.code(400).send("El Id " + enduranceId + " no es válido");
        return;
      }
      ...
      {
        reply.code(400).send("No existe ningun endurance con Id " + enduranceId);
        return;
      }
      // all checks passed ok
